            ...
            if conflicto:
                ...
                # Si se dio password de superusuario
                    ...
                    if superuser and superuser.is_superuser:
                        ...
                        form.add_error(
                            None,
                            "La autenticación de superusuario es incorrecta. No se creó la factura.",
                        )
                        print("[DEBUG] Autenticación superuser fallida.")
                else:
                    form.add_error(
                        None,
                        f"El cliente del {conflicto_tipo} seleccionado no coincide. Contacta al superusuario para autorizar el cambio.",
                    )
...
def facturar_mes_actual(request, facturar_locales=True, facturar_areas=True):
    ...
    folios_locales = set(
        Factura.objects.filter(
            empresa__in=[l.empresa for l in locales], folio__startswith="CM-F"
        ).values_list("empresa_id", "folio")
    )
    folios_areas = set(
        Factura.objects.filter(
            empresa__in=[a.empresa for a in areas], folio__startswith="AC-F"
        ).values_list("empresa_id", "folio")
    )
    folios_deposito = set(
        Factura.objects.filter(
            empresa__in=[a.empresa for a in areas], folio__startswith="DG-F"
        ).values_list("empresa_id", "folio")
    )
        ...
        if (local.cliente_id, local.pk) not in facturas_locales_existentes:
            ...
            if empresa_id not in max_folio_local:
                max_folio = Factura.objects.filter(
                    empresa_id=empresa_id, folio__startswith="CM-F"
    ...
    for area in areas:
        if (area.cliente_id, area.pk) not in facturas_areas_existentes:
            ...
            if empresa_id not in max_folio_area:
                max_folio = Factura.objects.filter(
                    empresa_id=empresa_id, folio__startswith="AC-F"
                ).aggregate(max_f=Max("folio"))["max_f"]
                if max_folio:
                    try:
        ...
        if area.deposito and area.deposito > 0:
            ...
            if not existe_deposito:
                ...
                if empresa_id not in max_folio_deposito:
                    max_folio = Factura.objects.filter(
                        empresa_id=empresa_id, folio__startswith="DG-F"
                    ).aggregate(max_f=Max("folio"))["max_f"]
                    if max_folio:
                        try:
    ...
    messages.success(
        request,
        f"{facturas_creadas} facturas generadas para {fecha_factura.strftime('%B %Y')}",
    )
    return redirect("lista_facturas")
...
def pagos_por_origen(request):
    ...
    if request.user.is_superuser:
        ...
        empresa = request.user.perfilusuario.empresa
        pagos = (
            Pago.objects.select_related("factura")
...
def dashboard_pagos(request):
    ...
    meses_facturas = sorted(
        set(f.fecha_vencimiento.replace(day=1) for f in facturas_pendientes)
    )
    ...
    for cobro in otros_cobros:
        for idx, mes in enumerate(todos_los_meses):
            if (
                cobro.fecha_cobro.year == mes.year
                and cobro.fecha_cobro.month == mes.month
            ):
...
def cartera_vencida(request):
    ...
    for f in facturas:
        ...
        f.tipo_origen = (
            "local"
            if getattr(f, "local_id", None)
            else "area" if getattr(f, "area_comun_id", None) else "cuota"
        )
        f.es_otro = False
    ...
    return render(
        request,
        "facturacion/cartera_vencida.html",
        {
            "facturas": page_obj,
            "hoy": hoy,
            "empresas": Empresa.objects.all(),
...
def exportar_cartera_excel(request):
    ...
    for factura in facturas:
        ...
        origen_str = (
            ...
            if factura.local
            else f"Área {factura.area_comun.numero}" if factura.area_comun else "-"
        )
        ws.append(
            [
                factura.folio,
    ...
    for factura in facturas_otros:
        dias_vencidos = (hoy - factura.fecha_vencimiento).days
        tipo_ingreso = (
            factura.get_tipo_ingreso_display()
            if hasattr(factura, "get_tipo_ingreso_display")
            else (
                factura.tipo_ingreso.nombre
                if hasattr(factura.tipo_ingreso, "nombre")
                else "Otro ingreso"
            )
        )
        ws.append(
                ...
                float(
                    getattr(
                        ...
                        (
                            ...
                            if hasattr(factura, "saldo_pendiente")
                            else factura.monto
                        ),
                    )
                ),
...
def crear_factura_otros_ingresos(request):
    ...
    return render(
        ...
        {"form": form, "tipos_ingreso": tipos_ingreso},
    )
...
def reporte_cobros_otros_ingresos(request):
    ...
    cobros = CobroOtrosIngresos.objects.select_related(
        "factura", "factura__empresa", "factura__cliente"
    )
